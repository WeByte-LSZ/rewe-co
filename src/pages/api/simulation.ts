...
export default function handler(
  ...
  for (const warehouse of warehouses) {
    ...
    console.log("im:" + warehouse.solarPanels);
    var solar_co2: number = warehouse.solarPanels ? co2WinSolarPanels(warehouse.area_m2) : 0;
...
function co2WinElectroTrucks(kWh: number) {
  return kWh * 0.5; //0.5 kg of CO2 per kWh electro trucks
}
...
function getVehicleCO2(co2EmissionRate: number, distance_km: number, additional_weight_kg: number, fuelConsumptionRate: number, isCooling: boolean) {
  ...
  var durchSchnittsVerbrauch = co2EmissionRate * -1;
  var kWh = distance_km * durchSchnittsVerbrauch;

  return co2WinElectroTrucks(kWh);
}
